...
cd $PARENT_DIR
rm -fv $PARENT_DIR/docker/ExamplePlugin

# docker build -f $PARENT_DIR/cross-compile/Dockerfile --no-cache -t example-plugin $PARENT_DIR
...
# docker cp example-plugin:/app/example-plugin/build/src/ExamplePlugin $PARENT_DIR/docker